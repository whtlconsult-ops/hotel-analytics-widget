'use client';

// ===== Block 1/4 =====
import React, { useEffect, useMemo, useState } from 'react';
import {
  format, parseISO, startOfMonth, endOfMonth, addDays, parse as parseDF
} from 'date-fns';
import { it } from 'date-fns/locale';
import {
  ResponsiveContainer, LineChart, Line, Area, XAxis, YAxis,
  CartesianGrid, Tooltip as RTooltip, BarChart, Bar
} from 'recharts';
// Se hai la mappa/leaflet:
import dynamic from 'next/dynamic';
const Map = dynamic(() => import('./Map'), { ssr: false });

/* ===================== Helpers ===================== */

// Giorni compresi nel mese YYYY-MM (inclusivi)
function daysOfMonthWindow(monthISO: string): Date[] {
  const anchor = parseDF(monthISO + '-01', 'yyyy-MM-dd', new Date());
  const s = startOfMonth(anchor);
  const e = endOfMonth(anchor);
  const out: Date[] = [];
  let d = s;
  while (d <= e) {
    out.push(d);
    d = addDays(d, 1);
  }
  return out;
}

// Giorni compresi tra due date ISO (inclusivi)
function daysBetween(fromISO: string, toISO: string): Date[] {
  const s = parseISO(fromISO);
  const e = parseISO(toISO);
  const out: Date[] = [];
  let d = s;
  while (d <= e) {
    out.push(d);
    d = addDays(d, 1);
  }
  return out;
}

// Ricampionamento “ultimo valore noto” (step-hold) su un set di date
function resampleHold(
  weekly: { date: string; score: number }[],
  dates: Date[]
): { dateISO: string; dateLabel: string; value: number }[] {
  if (!Array.isArray(weekly) || weekly.length === 0) {
    return dates.map(d => ({
      dateISO: d.toISOString().slice(0, 10),
      dateLabel: format(d, 'd MMM', { locale: it }),
      value: 0
    }));
  }
  const pts = weekly
    .map(p => ({ d: parseISO(p.date), v: Number(p.score) || 0 }))
    .sort((a, b) => a.d.getTime() - b.d.getTime());

  let idx = 0;
  let last = 0;

  return dates.map(day => {
    while (idx < pts.length && pts[idx].d.getTime() <= day.getTime()) {
      last = pts[idx].v;
      idx++;
    }
    return {
      dateISO: day.toISOString().slice(0, 10),
      dateLabel: format(day, 'd MMM', { locale: it }),
      value: last
    };
  });
}

// Normalizza una serie numerica 0..100 (evita divisioni per 0)
function normalize0to100(values: number[]): number[] {
  if (!values.length) return [];
  const min = Math.min(...values);
  const max = Math.max(...values);
  const den = Math.max(1, max - min);
  return values.map(v => Math.round(((v - min) / den) * 100));
}

// ADR stimato a partire da “pressione” (0..100) + uplift modalità competitor
function adrFromPressure(pressure01_100: number, mode: 'zone' | 'competitor'): number {
  const baseMin = 80;      // puoi tarare questi due numeri
  const baseMax = 140;
  const adr = baseMin + (pressure01_100 / 100) * (baseMax - baseMin);
  return Math.round(mode === 'competitor' ? adr * 1.1 : adr); // +10% in “competitor”
}

/* ===================== Tipi ===================== */

type RelatedBuckets = {
  channels: { label: string; value: number }[];
  provenance: { label: string; value: number }[];
  los: { label: string; value: number }[];
};

type SerpResp = {
  ok: boolean;
  series: { date: string; score: number }[];
  related?: RelatedBuckets;
  note?: string;
  meta?: { forecastFrom?: string };
};

type QuotaResp = {
  ok: boolean;
  total_searches_left?: number;
  plan_searches_left?: number;
  this_month_usage?: number;
};
// ===== Block 2/4 =====
export default function AppWidget() {
  /* -------- Stato UI base -------- */
  const [query, setQuery] = useState<string>('Firenze hotel');
  const [lat, setLat] = useState<number | undefined>(43.7696);
  const [lng, setLng] = useState<number | undefined>(11.2558);
  const [radiusKm, setRadiusKm] = useState<number>(20);

  // Mese per il CALENDARIO (YYYY-MM)
  const [monthISO, setMonthISO] = useState<string>(format(new Date(), 'yyyy-MM'));

  // Modalità (impatta ADR)
  const [mode, setMode] = useState<'zone' | 'competitor'>('competitor');

  // Toggle dei grafici secondari (consumano quote solo quando spuntati)
  const [wantTrend, setWantTrend] = useState<boolean>(true);
  const [wantCh, setWantCh] = useState<boolean>(true);
  const [wantProv, setWantProv] = useState<boolean>(true);
  const [wantLos, setWantLos] = useState<boolean>(true);

  // Range personalizzato per i GRAFICI (non il calendario)
  const [rangeEnabled, setRangeEnabled] = useState(false);
  const [rangeFrom, setRangeFrom] = useState<string>(''); // es. 2025-06-01
  const [rangeTo, setRangeTo] = useState<string>('');     // es. 2025-08-31

  // Dati Serp
  const [seriesRaw, setSeriesRaw] = useState<SerpResp['series']>([]);
  const [related, setRelated] = useState<RelatedBuckets | undefined>(undefined);
  const [note, setNote] = useState<string | undefined>(undefined);

  // Quota SerpAPI
  const [quota, setQuota] = useState<QuotaResp | null>(null);

  // UI busy
  const [loading, setLoading] = useState<boolean>(false);

  /* -------- Lettura stato da URL (se arrivi da link condiviso) -------- */
  useEffect(() => {
    const sp = new URLSearchParams(globalThis?.location?.search || '');
    const q = sp.get('q'); if (q) setQuery(q);
    const r = Number(sp.get('r') || 20); setRadiusKm([10,20,30].includes(r) ? r : 20);
    const m = sp.get('m'); if (m) setMonthISO(m);
    const modeS = sp.get('mode'); if (modeS === 'zone' || modeS === 'competitor') setMode(modeS);

    setWantTrend(sp.get('trend') !== '0');
    setWantCh(sp.get('ch') === '1');
    setWantProv(sp.get('prov') === '1');
    setWantLos(sp.get('los') === '1');

    const f = sp.get('from') || ''; const t = sp.get('to') || '';
    if (f && t) { setRangeEnabled(true); setRangeFrom(f); setRangeTo(t); }
  }, []);

  /* -------- Scrive lo stato nell’URL quando rigeneri -------- */
  function persistUrl() {
    const url = new URL(globalThis?.location?.href || '');
    const p = url.searchParams;
    p.set('q', query);
    p.set('r', String(radiusKm));
    p.set('m', monthISO);
    p.set('mode', mode);
    p.set('trend', wantTrend ? '1' : '0');
    p.set('ch', wantCh ? '1' : '0');
    p.set('prov', wantProv ? '1' : '0');
    p.set('los', wantLos ? '1' : '0');
    if (rangeEnabled && rangeFrom && rangeTo) {
      p.set('from', rangeFrom);
      p.set('to', rangeTo);
    } else {
      p.delete('from'); p.delete('to');
    }
    history.replaceState({}, '', `${url.pathname}?${p.toString()}`);
  }

  /* -------- Fetch QUOTA -------- */
  async function fetchQuota() {
    try {
      const r = await fetch('/api/serp/quota', { cache: 'no-store' });
      const j = await r.json();
      setQuota(j);
    } catch {
      // ignora
    }
  }

  /* -------- Fetch SERP (demand + related) -------- */
  async function fetchSerp() {
    setLoading(true);
    try {
      // from/to per il calendario se non c’è un range personalizzato
      const monthStart = format(startOfMonth(parseDF(monthISO + '-01', 'yyyy-MM-dd', new Date())), 'yyyy-MM-dd');
      const monthEnd   = format(endOfMonth(parseDF(monthISO + '-01', 'yyyy-MM-dd', new Date())), 'yyyy-MM-dd');

      const params = new URLSearchParams();
      params.set('q', query);
      if (Number.isFinite(lat as number) && Number.isFinite(lng as number)) {
        params.set('lat', String(lat));
        params.set('lng', String(lng));
      }
      // parti: trend sempre, related solo se serve
      const parts: string[] = [];
      if (wantTrend) parts.push('trend');
      if (wantCh || wantProv || wantLos) parts.push('related');
      params.set('parts', parts.join(',') || 'trend');

      // flag bucket
      if (wantCh) params.set('ch', '1');
      if (wantProv) params.set('prov', '1');
      if (wantLos) params.set('los', '1');

      // from/to: se range abil., usa quelli; altrimenti mese corrente (per calendario)
      if (rangeEnabled && rangeFrom && rangeTo) {
        params.set('from', rangeFrom);
        params.set('to', rangeTo);
      } else {
        params.set('from', monthStart);
        params.set('to', monthEnd);
      }

      // chiama backend robusto
      const r = await fetch(`/api/serp/demand?${params.toString()}`, { cache: 'no-store' });
      const j: SerpResp = await r.json();

      if (!j.ok) {
        setSeriesRaw([]);
        setRelated(undefined);
        setNote(j.note || 'Nessun dato disponibile per i parametri selezionati.');
      } else {
        setSeriesRaw(Array.isArray(j.series) ? j.series : []);
        setRelated(j.related);
        setNote(j.note);
      }
    } catch (e) {
      setSeriesRaw([]);
      setRelated(undefined);
      setNote('Errore nel recupero dati.');
    } finally {
      setLoading(false);
      persistUrl();
      fetchQuota();
    }
  }

  // Fetch iniziale
  useEffect(() => {
    fetchSerp(); // al mount
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
// ===== Block 3/4 =====
  /* -------- Derivate per CALENDARIO (mese) -------- */
  const monthDays = useMemo(() => daysOfMonthWindow(monthISO), [monthISO]);

  // Ricampiona serie su giorni del mese (se ho usato from/to = mese)
  const serpTrendMonth = useMemo(() => {
    const days = monthDays;
    return resampleHold(seriesRaw, days);
  }, [seriesRaw, monthDays]);

  // Pressione 0..100 per calendario (normalizzata nel mese)
  const calendarPressure = useMemo(() => {
    const vals = serpTrendMonth.map(p => p.value || 0);
    const norm = normalize0to100(vals);
    return norm;
  }, [serpTrendMonth]);

  // Dati calendario (pressione + ADR stimato)
  const calendarData = useMemo(() => {
    return monthDays.map((d, i) => ({
      dateISO: d.toISOString().slice(0, 10),
      day: format(d, 'd', { locale: it }),
      pressure: calendarPressure[i] || 0,
      adr: adrFromPressure(calendarPressure[i] || 0, mode),
    }));
  }, [monthDays, calendarPressure, mode]);

  /* -------- Derivate per RANGE personalizzato -------- */
  const rangeDays = useMemo(() => {
    if (rangeEnabled && rangeFrom && rangeTo) return daysBetween(rangeFrom, rangeTo);
    // altrimenti usa il mese corrente per il grafico
    const s = format(startOfMonth(parseDF(monthISO + '-01', 'yyyy-MM-dd', new Date())), 'yyyy-MM-dd');
    const e = format(endOfMonth(parseDF(monthISO + '-01', 'yyyy-MM-dd', new Date())), 'yyyy-MM-dd');
    return daysBetween(s, e);
  }, [rangeEnabled, rangeFrom, rangeTo, monthISO]);

  const serpTrendRangeDaily = useMemo(() => {
    return resampleHold(seriesRaw, rangeDays);
  }, [seriesRaw, rangeDays]);

  // Serie con ADR stimato per overlay (linea rossa)
  const trendWithADR = useMemo(() => {
    const vals = serpTrendRangeDaily.map(p => p.value || 0);
    const norm = normalize0to100(vals);
    return serpTrendRangeDaily.map((p, i) => ({
      ...p,
      adrEst: adrFromPressure(norm[i] || 0, mode)
    }));
  }, [serpTrendRangeDaily, mode]);

  /* -------- Quota badge -------- */
  const quotaBadge = useMemo(() => {
    if (!quota?.ok) return '—/—';
    const left = quota.total_searches_left ?? quota.plan_searches_left ?? undefined;
    const used = quota.this_month_usage ?? undefined;
    if (left == null || used == null) return '—/—';
    return `${used}/${used + left} (left ${left})`;
  }, [quota]);

  /* -------- UI -------- */
  const disabledGenerate = loading || !query?.trim();

  return (
    <div className="min-h-screen bg-slate-50">
      {/* Topbar */}
      <header className="sticky top-0 z-10 bg-white/90 backdrop-blur border-b">
        <div className="max-w-6xl mx-auto px-4 py-3 flex flex-wrap items-center gap-3">
          <div className="text-sm px-2 py-1 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-200">
            SerpAPI quota: {quotaBadge}
          </div>
          <div className="flex-1" />
          <div className="text-[12px] text-slate-500">{note || ''}</div>
        </div>
      </header>

      <main className="max-w-6xl mx-auto px-4 py-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Sidebar controlli */}
        <aside className="lg:col-span-4 space-y-4">
          <section className="bg-white rounded-2xl border shadow-sm p-4 space-y-3">
            <div className="text-sm font-semibold">Località & Parametri</div>
            <input
              value={query}
              onChange={e => setQuery(e.target.value)}
              placeholder="Es. Firenze hotel"
              className="w-full h-10 px-3 rounded-xl border border-slate-300"
            />
            <div className="grid grid-cols-2 gap-2">
              <label className="text-xs text-slate-600">Raggio (km)</label>
              <select
                value={radiusKm}
                onChange={e => setRadiusKm(Number(e.target.value))}
                className="h-9 px-2 rounded-xl border border-slate-300"
              >
                <option value={10}>10</option>
                <option value={20}>20</option>
                <option value={30}>30</option>
              </select>
              <label className="text-xs text-slate-600">Mese (calendario)</label>
              <input
                type="month"
                value={monthISO}
                onChange={(e) => setMonthISO(e.currentTarget.value)}
                className="h-9 px-2 rounded-xl border border-slate-300"
              />
              <label className="text-xs text-slate-600">Modalità</label>
              <select
                value={mode}
                onChange={e => setMode(e.target.value as any)}
                className="h-9 px-2 rounded-xl border border-slate-300"
              >
                <option value="zone">Zona</option>
                <option value="competitor">Competitor</option>
              </select>
            </div>
          </section>

          <section className="bg-white rounded-2xl border shadow-sm p-4 space-y-2">
            <div className="text-sm font-semibold">Selettori API (risparmio quote)</div>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={wantTrend} onChange={e => setWantTrend(e.currentTarget.checked)} />
              Andamento domanda (Trends)
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={wantCh} onChange={e => setWantCh(e.currentTarget.checked)} />
              Canali di vendita
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={wantProv} onChange={e => setWantProv(e.currentTarget.checked)} />
              Provenienza clienti
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={wantLos} onChange={e => setWantLos(e.currentTarget.checked)} />
              Durata soggiorno (LOS)
            </label>
          </section>

          <section className="bg-white rounded-2xl border shadow-sm p-4 space-y-3">
            <div className="text-sm font-semibold">Periodo personalizzato (solo grafici)</div>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={rangeEnabled} onChange={(e) => setRangeEnabled(e.currentTarget.checked)} />
              Abilita intervallo
            </label>
            {rangeEnabled && (
              <div className="grid gap-2">
                <div className="flex items-center gap-2">
                  <span className="w-14 text-xs text-slate-600">Da</span>
                  <input
                    type="date"
                    className="h-9 rounded-xl border border-slate-300 px-2 text-sm"
                    value={rangeFrom}
                    onChange={(e) => setRangeFrom(e.target.value)}
                  />
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-14 text-xs text-slate-600">A</span>
                  <input
                    type="date"
                    className="h-9 rounded-xl border border-slate-300 px-2 text-sm"
                    value={rangeTo}
                    onChange={(e) => setRangeTo(e.target.value)}
                  />
                </div>
                <p className="text-[11px] text-slate-500">
                  Il calendario resta mensile sul mese selezionato. I grafici usano il range sopra.
                </p>
              </div>
            )}
          </section>

          <button
            onClick={fetchSerp}
            disabled={disabledGenerate}
            className={`w-full h-11 rounded-2xl text-white text-sm font-semibold ${disabledGenerate ? 'bg-slate-300' : 'bg-indigo-600 hover:bg-indigo-700'}`}
          >
            {loading ? 'Elaboro…' : 'Genera Analisi'}
          </button>

          {/* Mappa (click per aggiornare lat/lng se il tuo Map lo supporta) */}
          <section className="bg-white rounded-2xl border shadow-sm p-2">
            <Map
              lat={lat ?? 43.7696}
              lng={lng ?? 11.2558}
              radiusKm={radiusKm}
              onPick={(p: { lat: number; lng: number; label?: string }) => {
                setLat(p.lat); setLng(p.lng);
                if (p.label && !/hotel/i.test(query)) setQuery(p.label + ' hotel');
              }}
            />
          </section>
        </aside>
        {/* Area grafici */}
        <section className="lg:col-span-8 space-y-6">
          {/* Calendario mensile (colore = pressione da Trends) */}
          <div className="bg-white rounded-2xl border shadow-sm p-4">
            <div className="text-sm font-semibold mb-2">
              Calendario domanda — {format(parseDF(monthISO + '-01', 'yyyy-MM-dd', new Date()), 'MMMM yyyy', { locale: it })}
            </div>
            <div className="grid grid-cols-7 gap-2">
              {calendarData.map((d, i) => {
                const color = `hsl(${(d.pressure / 100) * 0 + 10}, 85%, ${90 - Math.round(d.pressure * 0.5)}%)`; // gamma chiara→scura
                return (
                  <div key={d.dateISO} className="rounded-lg p-2 border text-center" style={{ background: color }}>
                    <div className="text-xs font-semibold">{d.day}</div>
                    <div className="text-[10px]">ADR ~ {d.adr}€</div>
                  </div>
                );
              })}
            </div>
          </div>

          {/* Andamento domanda sul MESE (linea) */}
          <div className="bg-white rounded-2xl border shadow-sm p-4">
            <div className="text-sm font-semibold mb-2">
              Andamento domanda — {format(parseDF(monthISO + '-01', 'yyyy-MM-dd', new Date()), 'MMMM yyyy', { locale: it })}
            </div>
            {serpTrendMonth.every(p => (p.value || 0) === 0) ? (
              <div className="h-56 flex items-center justify-center text-sm text-slate-500">
                Dati insufficienti per il mese selezionato.
              </div>
            ) : (
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={serpTrendMonth}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="dateLabel" interval={Math.ceil(serpTrendMonth.length / 12)} />
                    <YAxis />
                    <RTooltip />
                    <defs>
                      <linearGradient id="grad1" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor="#1e3a8a" />
                        <stop offset="100%" stopColor="#1e3a8a" />
                      </linearGradient>
                      <linearGradient id="fill1" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor="#1e3a8a" stopOpacity={0.2} />
                        <stop offset="100%" stopColor="#1e3a8a" stopOpacity={0.03} />
                      </linearGradient>
                    </defs>
                    <Area type="monotone" dataKey="value" fill="url(#fill1)" stroke="url(#grad1)" />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            )}
          </div>

          {/* Periodo PERSONALIZZATO scrollabile: Domanda + ADR overlay */}
          <div className="bg-white rounded-2xl border shadow-sm p-4">
            <div className="text-sm font-semibold mb-2">
              Andamento (range) — {rangeEnabled && rangeFrom && rangeTo ? `${rangeFrom} → ${rangeTo}` : 'mese corrente'}
            </div>
            {trendWithADR.length === 0 || trendWithADR.every(p => (p.value || 0) === 0) ? (
              <div className="h-56 flex items-center justify-center text-sm text-slate-500">
                Nessun segnale per il periodo selezionato.
              </div>
            ) : (
              <div className="overflow-x-auto">
                <div style={{ minWidth: `${Math.max(trendWithADR.length * 14, 720)}px` }}>
                  <ResponsiveContainer width="100%" height={320}>
                    <LineChart data={trendWithADR}>
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis dataKey="dateLabel" interval={Math.ceil(trendWithADR.length / 14)} />
                      <YAxis yAxisId="demand" />
                      <YAxis yAxisId="adr" orientation="right" />
                      <RTooltip />
                      {/* area domanda */}
                      <defs>
                        <linearGradient id="grad2" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="0%" stopColor="#1e3a8a" />
                          <stop offset="100%" stopColor="#1e3a8a" />
                        </linearGradient>
                        <linearGradient id="fill2" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="0%" stopColor="#1e3a8a" stopOpacity={0.2} />
                          <stop offset="100%" stopColor="#1e3a8a" stopOpacity={0.03} />
                        </linearGradient>
                      </defs>
                      <Area yAxisId="demand" type="monotone" dataKey="value" fill="url(#fill2)" stroke="url(#grad2)" />
                      {/* linea ADR stimato */}
                      <Line yAxisId="adr" type="monotone" dataKey="adrEst" stroke="#ef4444" strokeWidth={1.6} dot={false} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </div>
            )}
            <div className="mt-2 text-[11px] text-slate-500">
              ADR stimato (rosso). Per ADR reale collega prezzi OTA/CSV/Sheet.
            </div>
          </div>

          {/* Grafici segmenti (Bar) */}
          {(wantCh || wantProv || wantLos) && (
            <div className="grid md:grid-cols-3 gap-4">
              {wantCh && (
                <div className="bg-white rounded-2xl border shadow-sm p-4">
                  <div className="text-sm font-semibold mb-2">Canali di vendita</div>
                  {!related?.channels?.length ? (
                    <div className="h-40 flex items-center justify-center text-sm text-slate-500">N/D</div>
                  ) : (
                    <div className="h-56">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={related.channels}>
                          <CartesianGrid strokeDasharray="3 3" />
                          <XAxis dataKey="label" />
                          <YAxis allowDecimals={false} />
                          <RTooltip />
                          <Bar dataKey="value" />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  )}
                </div>
              )}
              {wantProv && (
                <div className="bg-white rounded-2xl border shadow-sm p-4">
                  <div className="text-sm font-semibold mb-2">Provenienza clienti</div>
                  {!related?.provenance?.length ? (
                    <div className="h-40 flex items-center justify-center text-sm text-slate-500">N/D</div>
                  ) : (
                    <div className="h-56">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={related.provenance}>
                          <CartesianGrid strokeDasharray="3 3" />
                          <XAxis dataKey="label" />
                          <YAxis allowDecimals={false} />
                          <RTooltip />
                          <Bar dataKey="value" />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  )}
                </div>
              )}
              {wantLos && (
                <div className="bg-white rounded-2xl border shadow-sm p-4">
                  <div className="text-sm font-semibold mb-2">Durata soggiorno (LOS)</div>
                  {!related?.los?.length ? (
                    <div className="h-40 flex items-center justify-center text-sm text-slate-500">N/D</div>
                  ) : (
                    <div className="h-56">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={related.los}>
                          <CartesianGrid strokeDasharray="3 3" />
                          <XAxis dataKey="label" />
                          <YAxis allowDecimals={false} />
                          <RTooltip />
                          <Bar dataKey="value" />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  )}
                </div>
              )}
            </div>
          )}
        </section>
      </main>

      <footer className="py-6 text-center text-[11px] text-slate-500">
        Demo analitica — Domanda da Google Trends via SerpAPI. Dati indicativi.
      </footer>
    </div>
  );
}


